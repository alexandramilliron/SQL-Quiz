...
SELECT COUNT(order_total) FROM orders WHERE shipto_state = 'CA';
...
SELECT orders.id, status, order_total FROM orders JOIN customers ON (customers.id = orders.customer_id) 
